    ...
    (.) = dot
        where 
            (Circuit cir2) `dot` (Circuit cir1) = Circuit $ \a ->
                let (cir1', b) = cir1 a
                    (cir2', c) = cir2 b
                in (cir2' `dot` cir1', c) -- this dot call looks like infinite recursion. ...?

instance Arrow Circuit where 
    arr f = Circuit $ \a -> (arr f, f a) -- arr f = ... arr f ... -- this looks like more infinite recursion ?_?
    first (Circuit cir) = Circuit $ \(b, d) ->
        let (cir', c) = cir b
         in (first cir', (c, d))

-- has n independent inputs, run it on n different dependent functions. why?
...
getWord rng = proc () -> do
    ...
    mWord <- accum' Nothing mplus -< mPicked -- Monad Plus thingy, keeps Just
